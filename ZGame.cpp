...
//	AdjustGlobalTime();
	...
	if(m_pMyCharacter && !m_bReplaying.Ref())
	{
		...
		if(ZGetGame()->GetMatch()->GetMatchType() != MMATCH_GAMETYPE_DUELTOURNAMENT)
			...
			PostDuelTournamentHPAPInfo(); // �����ʸ�Ʈ ������ ó�����ش�.
...
{
//		 if(gid == MMUG_FREE)			{ UserNameColor = MCOLOR(200,200,200); return true; }// ��������
//	else if(gid == MMUG_REGULAR)		{ UserNameColor = MCOLOR(200,200,200); return true; }// ��������
//	else if(gid == MMUG_CRIMINAL)		{ UserNameColor = MCOLOR(200,200,200); return true; }// ������
//	else if(gid == MMUG_WARNING_1)		{ UserNameColor = MCOLOR(200,200,200); return true; }// 1�����
//	else if(gid == MMUG_WARNING_2)		{ UserNameColor = MCOLOR(200,200,200); return true; }// 2�����
//	else if(gid == MMUG_WARNING_3)		{ UserNameColor = MCOLOR(200,200,200); return true; }// 3�����
//	else if(gid == MMUG_CHAT_LIMITED)	{ UserNameColor = MCOLOR(200,200,200); return true; }// ä�� ����
//	else if(gid == MMUG_PENALTY)		{ UserNameColor = MCOLOR(200,200,200); return true; }// �Ⱓ ����
//	else if(gid == MMUG_BLOCKED)		{ UserNameColor = MCOLOR(200,200,200); return true; }// �������
			...
			if(pChar) 
			{
				...
				if ( (nTeam == MMT_ALL) || (nTeam == MMT_SPECTATOR))
				{
					if ( !ZGetGameClient()->GetRejectNormalChat() || ( strcmp( pChar->GetUserName(), ZGetMyInfo()->GetCharName()) == 0))
					{
						...
						if ( ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DUEL)	// ��� ��ġ�϶�
						{
...
*/
	...
	case MC_GUNZ_ANTILEAD:
	{
		...
		if (ZGetChannelRuleMgr()->GetCurrentRule() && ZGetChannelRuleMgr()->GetCurrentRule()->GetID() == MCHANNEL_RULE_LEAD) break;

		ZCharacter* pCharacter = (ZCharacter*)ZGetCharacterManager()->Find(pCommand->GetSenderUID());
		if (!pCharacter) break;

		uint32_t ZDamageInfo = 0;

		for (int i = 0; i < nSize; ++i)
		{
			ZShotInfo* pInfo = (ZShotInfo*)MGetBlobArrayElement(pBlob, i);
			if (!pInfo) break;
			...
			if (m_pMyCharacter && ZGetGameClient()->GetPlayerUID().Low != pCommand->GetSenderUID().Low)
				...
				{
					float fDamageCalc = m_pMyCharacter->OnDamagedCalc(pCharacter, pCharacter->GetPosition(), dt, wt, fDamage, fRatio);
					ZDamageInfo += fDamageCalc;
					m_pMyCharacter->OnDamaged(pCharacter, pCharacter->GetPosition(), dt, wt, fDamage, fRatio);
				}
		}

		if (ZDamageInfo > 0 && pCharacter != NULL && ZGetGameClient()->GetMatchStageSetting()->GetGameType() != MMATCH_GAMETYPE_QUEST && ZGetGameClient()->GetMatchStageSetting()->GetGameType() != MMATCH_GAMETYPE_SURVIVAL)
		{
			ZPostDamageCounter(ZDamageInfo, pCharacter->GetUID());
		}
		break;
	}
	case MC_GUNZ_DAMAGECOUNTER:
	{
		...
		ZCharacter* pVictimCharacter = (ZCharacter*)ZGetCharacterManager()->Find(senderUID);
		if (pCharacter)
		{
			pCharacter->nDamageCount += Damage;
		}

		if (pVictimCharacter)
		{
			pVictimCharacter->nTakenDamage += Damage;
			pVictimCharacter->nBossDamage += Damage;
		}
	}
		break;
...
#define SHOTGUN_DIFFUSE_RANGE	0.1f
	...
	for (int i = 0; i < SHOTGUN_BULLET_COUNT; i++)
	{
		...
		const DWORD dwPickPassFlag = RM_FLAG_ADDITIVE | RM_FLAG_HIDE | RM_FLAG_PASSROCKET | RM_FLAG_PASSBULLET;
		//jintriple3 ����� �������� �� ��� �Լ�..
		ZShotInfo* pShotInfo = OnPeerShotgun_Damaged(pOwnerCharacter, fShotTime, pos, dir, pickinfo, dwPickPassFlag, v1, v2, pItem, BulletMarkNormal, bBulletMark, nTargetType, bHitEnemy);
...
//jintriple3 ����� �������� ��ŷ ���� �ڵ� ����
ZShotInfo* ZGame::OnPeerShotgun_Damaged(ZObject* pOwner, float fShotTime, const rvector& pos, rvector& dir, ZPICKINFO pickinfo, DWORD dwPickPassFlag, rvector& v1, rvector& v2, ZItem* pItem, rvector& BulletMarkNormal, bool& bBulletMark, ZTargetType& nTargetType, bool& bHitEnemy)
{
	ZCharacter* pTargetCharacter = ZGetGameInterface()->GetCombatInterface()->GetTargetCharacter();
...
{
	const char *testdeathnametable[ZD_END+1] = { "����", "��", "Į", "�߶�", "����", "HEADSHOT", "������Į��" };
	char szMsg[256] = "";

	const char *szAnonymous = "?�ƹ���?";
...
//			sprintf(szMsg, "%s���� ������ �й��Ͽ����ϴ�.", szAttacker);
	...
	{
//		sprintf(szMsg, "����� %s�����κ��� �¸��Ͽ����ϴ�.", szVictim );
	...
	{
//		sprintf(szMsg, "����� %s�Կ��� �й��Ͽ����ϴ�.", szAttacker );
...
void ZGame::OnPeerSpawn(MUID& uid, rvector& pos, rvector& dir)
{
	...
	if (ZGetConfiguration()->GetEtc()->bDisplayDamage)
	{
		pCharacter->nDamageCount = 0;
	}
...
if((nNowTime - nLastTime) < 100) return;	// 100�и������帶�� üũ
...
if(pCharacter->m_BasicHistory.size()==0) continue;		// ��ȿ���� �ʴ�
...
fAverageTime=max(fAverageTime,0);	// 0���� �������� ����.
...
#ifndef _PUBLISH
char szTemp[256];
sprintf(szTemp, "%s���� ���ǵ��� ? %3.1f", pCharacter->GetProperty()->szName,pCharacter->m_fAccumulatedTimeError);
...
/*
	...
	if( strcmp(g_pGame->m_pMyCharacter->GetProperty()->szName,"����5")!=0 )
		return;
...
void ZGame::AdjustMyData()
{
	...
	{
		ZCharacter *pCharacter=i->second;
		if(pCharacter->m_HPHistory.size()==0) return;	// �ѳ��̶� �����Ͱ� ������ ��ġ�ʴ�.
	...
	for(i=m_CharacterManager.begin();i!=m_CharacterManager.end();i++)
	{
		...
		if(pCharacter->m_nReceiveHPCount && (GetTime()-pCharacter->GetSpawnTime())>3.f)
		{
			...
			if(pCharacter==m_pMyCharacter)	// �� ĳ���ʹ� �Ǹ� �ø����� �ʴ´�.
			{
				pCharacter->SetHP(
...
void ZGame::PostBasicInfo()
{
	...
	if ((int)(nNowTime - m_nLastTime[ZLASTTIME_BASICINFO]) >= nMoveTick)
	{
		...
		pbi.posz = (int(m_pMyCharacter->GetPosition().z * 0.1f)) * 10.f;*/	// ������ ���� ���������� ���� �ϱ� ���� 1�� �ڸ����� ������ ����
	...
	case MMATCH_ROUNDSTATE_PLAY:
		...
		MMATCH_GAMETYPE gameType = ZGetGameClient()->GetMatchStageSetting()->GetGameType();
		 if (gameType == MMATCH_GAMETYPE_DUEL ||
			gameType == MMATCH_GAMETYPE_DUELTOURNAMENT ||
			gameType == MMATCH_GAMETYPE_QUEST ||
			gameType == MMATCH_GAMETYPE_SURVIVAL ||
			gameType == MMATCH_GAMETYPE_DEATHMATCH_TEAM ||
			gameType == MMATCH_GAMETYPE_DEATHMATCH_TEAM2 ||
			...
			)
		{
			if (ZGetConfiguration()->GetEtc()->bDisplayDamage)
			{
				...
				sprintf(FinishStr, "^1Damage Done: %i / Damage Received: %i", m_pMyCharacter->nDamageCount, m_pMyCharacter->nTakenDamage);
