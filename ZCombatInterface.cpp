...
void ZCombatInterface::DamageFloatingEffect(MDrawContext* pDC)
{
	if (ZGetGame() && ZGetConfiguration()->GetEtc()->bFloatdmg)
	{
		...
		int nCurrDamage = pCharacter->GetStatus().Ref().nDamageCaused;            //nRoundGivenDamage;
		int Shots = ZGetGame()->m_pMyCharacter->m_nShots;
		int Hits = ZGetGame()->m_pMyCharacter->m_nHits;
		if (Hits > 0 && Shots > 0)
		{
			nCurrDamage = nCurrDamage / Hits;
		}

		if (nCurrDamage != nLastDamage)
		{
			nLastDamage = nCurrDamage;
			ZGetScreenEffectManager()->AddDamageEffect(nLastDamage);
		}
	}
}
...
void ZCombatInterface::OnDraw(MDrawContext* pDC)
{
	...
	if(IsShowUI())				// ��� UI ���߱�... by kam 20081020
	{
		...
		if (!m_bSkipUIDrawByRule)
		{
			...
			if (ZGetConfiguration()->GetEtc()->bDisplayFPS)
			{
				// FPS Display
				char szMsg[128] = { 0, };
				sprintf(szMsg, "FPS: %.f", g_fFPS);
				pDC->SetFont(MFontManager::Get("FONTa10_O2Wht"));
				pDC->Text(RGetScreenWidth() - 155, 15, szMsg);
			}
			...
			if (ZGetConfiguration()->GetEtc()->bDisplayDamage)
			{
				...
				char szText[512];
				//  Damage Display
				sprintf(szText, "Damage Done: %i / Damage Received: %i", pCharacter->nDamageCount, pCharacter->nTakenDamage);
				TextRelative(pDC, 10.f / 800.f, 105.f / 600.f, szText);
			}
...
//	const float clanXPOS[6] = {0.45f, 0.25f, 0.68f, 0.77f, 0.83f, 0.92f};
	...
	//if ( ZGetGameTypeManager()->IsQuestDerived( ZGetGame()->GetMatch()->GetMatchType()))
	if (ZGetGameTypeManager()->IsQuestDerived(ZGetGame()->GetMatch()->GetMatchType()) || ZGetGame()->GetMatch()->IsTeamPlay())
	{
		x = ITEM_XPOS[2];	// HP/AP
	...
	else
	{
		...
		x = ITEM_XPOS[2] - .01f;// DAMAGE
		TextRelative(pDC, x, y, "Damage");
	}
...
//	int nMaxLineCount=int((fHeight-fTitleHeight)*(float)RGetScreenHeight()/((float)pFont->GetHeight()*1.1f));
	...
	//	pItem->nDamageCaused = pCharacter->nDamageCount;
	...
	if(bClanGame)
	{
		...
		for(ZResultBoardList::iterator i=m_ResultItems.begin();i!=m_ResultItems.end();i++)
		{
			...
			if(bDrawClanName)
			{
				...
				TextRelative(pDC,x+.25f,texty,"Damage",true);
