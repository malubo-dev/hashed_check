...
#define END_(x) { _end_time = timeGetTime(); float f_time = (_end_time - _begin_time) / 1000.f; mlog("%s : %f \n", x,f_time ); }
	...
	m_pWeaponIcons[MWT_GREAT_SWORD]		= new ZScreenEffect(m_pEffectMeshMgr->Get("ef_in_icon_sword.elu"));//�׳� �켱 ������ ���
	m_pWeaponIcons[MWT_DOUBLE_KATANA]	= new ZScreenEffect(m_pEffectMeshMgr->Get("ef_in_icon_blade.elu"));//�׳� �켱 ������ ���
...
void ZScreenEffectManager::UpdateEffects()
{
	...
	{ // ZGetGameInterface()->GetCombatInterface() <- NULL�� ��찡 ���ö��� ����
...
void ZScreenEffectManager::SetCombo(int nCombo)
{
	...
	if (nCombo > MAX_COMBO) nCombo = MAX_COMBO;		// 99 ������ ��������
...
// Float Damage
void ZScreenEffectManager::AddDamageEffect(int nDmg)
{
#define EXP_NUMBER_SPACE	30.f
	...
	{
		float fOffset = EXP_NUMBER_SPACE * (float)(i - nCount + 2);
		Add(new ZScreenEffect(m_pDmgNumberEffect[buffer[i] - '0'], rvector(fOffset, 0, 0)));
	}

	float fOffset = EXP_NUMBER_SPACE * (float)(3 - nCount);
	Add(new ZScreenEffect(nDmg > 0 ? m_pDmgPlusEffect : m_pDmgMinusEffect, rvector(fOffset, 0, 0)));
}
...
{
#ifdef _BIRDSOUND
	ZGetSoundEngine()->PlaySound("if_score_flyby");		// ���� ������ ����ȵǾ� ���� 
#else
